...
1. Copy the header-only library (marching_cubes_33_c.h file) into your code folder. You need to define marching_cubes_33_c_implementation in only one of your code files before including marching_cubes_33_c.h (you can include marching_cubes_33_c.h in multiple files without issue):
	```c
	#define marching_cubes_33_c_implementation // only once in your project
	#include "marching_cubes_33_c.h"
	```
2. You can declare a `_GRD` pointer and then use one of the functions to load files that contain grids (`read_grd`, `read_scanfiles`, `read_raw_file` or `read_dat_file`):
```c
	_GRD* G;
...
There are some options that can be modified. You can do it by editing the "CUSTOMIZING" section in the marching_cubes_33_c.h file:

1. To change the data type of the grid (the default value is float) define GRD_TYPE_SIZE and/or GRD_INTEGER. For example:
	```c
...
In the GLUT example, the file containing the grid must be passed to the program on the command line (you can also drag and drop the grid file into the executable file in Windows File Explorer). No other grid files can be read from the running program. This example uses the `generate_grid_from_fn` function to generate grids from math functions if the grid file is not specified.
